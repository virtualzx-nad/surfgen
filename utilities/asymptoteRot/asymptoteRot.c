...
/* Change Log------
   10-08-2014 - Added dynamic allocation of arrays
 */
#include <stdio.h>
#include <math.h>
#include <stdlib.h>
...
{
  double *gStart;  /* Starting geometry */
  double *gOrigin; /* Shifted  geometry */
  double *sVector; /* Shift    vector   */

  printHeader(); /* print header information */
  readInput();	 /* read input information   */
    
  /* Allocate gStart, gOrigin, sVector */
  gStart = malloc((natoms*3)*sizeof(double));
  gOrigin= malloc((natoms*3)*sizeof(double));
  sVector= malloc((natoms*3)*sizeof(double));
  
  readGeomFile(gStart);	  /* Read in geometry file        */
  printGeomFile(gStart);  /* Print original geometry file */
		
  ShiftGeom(gStart,gOrigin,sVector);  /* Shift Geometry, also returns shift vector */
  rotationGen1(gOrigin,sVector);      /* Perform Rotations */
...
void rotationGen1( double *gP, double *ShiftVector )
{
  ...
  double *rotVecs;						// Rotation vectors are stored in same array
  double *rMP; /* rotationMatrix */
  double rotints; /* rotation intervals */
  int i,j;

  /* Allocate array */
  rotVecs = malloc(9*sizeof(double));
  ...
  rMP = malloc(9*sizeof(double));  /* rotation Matrix */
...
void PerfRotation(double *RotMatP, double *MolGeoP, int disp, double *ShiftVector, int choice )
{
  ...
  gSCr = &geomScratch[0]; // Set pointer
  for ( i=1; i<=disp; i++ ){
    nVec = &newVec[0];
    MatVec( 3, 3, RotMatP, (gSCr+(AtomSpec-1)*3), nVec );		// Perform Matrix multplication ( Rotation )
    RepCoord( gSCr, newVec );					// Replace Coordinates of Special atom
    WriteFile( gSCr, ShiftVector, i, choice );
  }
}
...
{
  int i;	
  char DirName[25], FileName[25];
  FILE *gfP;
  double *gPP; /* Geometry to print */

  /* Allocate geometry to print */
  if( NULL == (gPP = malloc((natoms*3)*sizeof(double))) ){
  ...
  for (i=0; i<natoms; i++ ){
    gPP[i*3+0] = Geometry[i*3+0]+ShiftVector[0];
    gPP[i*3+1] = Geometry[i*3+1]+ShiftVector[1];
    gPP[i*3+2] = Geometry[i*3+2]+ShiftVector[2];
  }	
#ifdef DEBUG
  printf( "Printing Shift Vector\n");
  for (i=0; i<3; i++ ){
    printf( "%lf\n", ShiftVector[i*3+0] );
  }
#endif
...
#ifdef DEBUG
  printf( "Printing Shifted Geometry\n");
  for (i=0; i<natoms; i++ ){
    printf( "%20.15lf ", gPP[(i*3)+0] );
    printf( "%20.15lf ", gPP[(i*3)+1] );    
    printf( "%20.15lf\n", gPP[(i*3)+2] );
  }
#endif
  ...
  for ( i = 0; i<natoms; i++ ){
    fprintf(gfP, "%20.15lf ", gPP[(i*3)+0] );
    fprintf(gfP, "%20.15lf ", gPP[(i*3)+1] );
    fprintf(gfP, "%20.15lf\n", gPP[(i*3)+2] ); 
  }
...
void RotVec1(double *aP, double *bP, double *hP )
{
  ...
  norm = sqrt(pow(hP[0],2) + pow(hP[1],2) + pow(hP[2],2));
...
void RotVec2( double *vector1, double *vector2, double *Output )
{
  ...
  norm = sqrt(pow(Output[0],2) + pow(Output[1],2) + pow(Output[2],2));
