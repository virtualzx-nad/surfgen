...
! allocate arrays
  ...
  do i=1,npts
    ...
    print *,""
    print *,"Cartesian Gradients and Couplings (multiplied by Ediff) in Adiabatic Representation"
    do j=1,nstates
      do k=1,j
         print "(A,I3,A,I3,A,e13.5)"," block (",j,",",k,"),|F|=",dnrm2(3*natoms,cg(1,j,k),1)
         if ( j .ne. k ) then
             print "(3F18.10)",cg(:,j,k)*abs((e(j)-e(k)))
         else
    ...
    print *,""
    print *, "Cartesian Gradients and Couplings (NOT mutltiplied by Ediff in Adiabatic Representation"
    do j=1, nstates
       do k=1, j
          print "(a,i3,a,i3,a,e15.8)"," block (",j,",",k,"),|F|=",dnrm2(3*natoms,cg(1,j,k),1)
          if ( j .ne. k ) then
             print "(3e18.10)",cg(:,j,k)
          else
             print "(3e18.10)",cg(:,j,k)
          end if
       end do
