...
C                        
      CALL GETSIZ(ICORE)
      CALL GETMEM(ICORE,IRS8,IAP,IFAIL)
C      CALL GETMEM(ICORE,IRS8,IAP)
      IF(IFAIL.NE.0)GO TO 900
      NCOR=ICORE-1
      IAP=(IAP+2)/2
C                                                       
      NPFLG=2 
      CALL SYSTIME(XT0,XU0,XS0)
      WRITE(6,*)' POLYHES CLOCK STARTS: ',FDATE()
      CALL VMAIN(RS8(IAP),RS8(IAP),NCOR)
      WRITE(6,*)' POLYHES CLOCK ENDS:   ',FDATE() 
      CALL SYSTIME(XT1,XU1,XS1)
C
      XT=XT1-XT0
      XU=XU1-XU0
      XS=XS1-XS0
...
C
 ...
 1001 FORMAT(/2X,'MEMORY LOCATION PARAMETERS:MATOMS=',I3,' N3=',I5,
     ...
     2       ,ACROSS,FIXDIR(6,30),KLASS(MFIXDIR),ACTUAL
...
C
  ...
  130 CONTINUE
C
      CALL RDT30(ICIU1,EVAL,NRTU1,IOUT,IERR)
      NCIU=ICIU1
      IF(IERR.NE.0)GO TO 900
      EJMEIA=EVAL(STATE1)
      ES1=EVAL(STATE1)
      IF(.NOT.SAME)CALL RDT30(ICIU2,EVAL,NRTU2,IOUT,IERR)
      NCIU=ICIU2
      IF(IERR.NE.0)GO TO 900
...
C         GET GEOM AND GRADIENT POINTERS
      CALL WREADW(ITAP10,ISTART,1,500+NCALC,JUNK)
      IPNTG=ISTART+80
      IPNTEN=IPNTG+INTOWP(NATOMS*3)
      CALL WREADW(ITAP10,ENUC,INTOWP(1),IPNTEN,JUNK)
      WRITE(IOUT,1035)ES1,ES2,ENUC
      IPNTN=ISTART+60+NACOUT-1
      CALL WREADW(ITAP10,IPNTN,1,IPNTN,JUNK)
      WRITE(IOUT,1010)IPNTG,IPNTN
C         GET RAW GRADIENTS
      IF(IPNTN.EQ.0)THEN
      WRITE(IOUT,1024)ITAP10
      CALL EXIT(100)
      ENDIF
      CALL WREADW(ITAP10,NACME,INTOWP(3*NATOMS),IPNTN,JUNK)
      WRITE(IOUT,1003)'ENERGY',(RNACME(I),I=1,3*NATOMS)
C
C          ENERGY DIFFERENCE GRADIENTS
      CALL RFILE(ITAP11)
C         GET GEOM AND GRADIENT POINTERS
      CALL WREADW(ITAP11,ISTART,1,500+NCALC,JUNK)
      IPNTG=ISTART+80
      IPNTN=ISTART+60+NACOUT-1
      CALL WREADW(ITAP11,IPNTN,1,IPNTN,JUNK)
      WRITE(IOUT,1010)IPNTG,IPNTN
C         GET RAW GRADIENTS
      IF(IPNTN.EQ.0)THEN
      WRITE(IOUT,1024)ITAP11
      CALL EXIT(100)
      ENDIF
      CALL WREADW(ITAP11,MACME,INTOWP(3*NATOMS),IPNTN,JUNK)
      WRITE(IOUT,1003)'E-DIF ',(RMACME(I),I=1,3*NATOMS)
C
      CALL RZERO(LACME,3*NATOMS)
      IF(SAME)THEN
C          H(1,2) GRADIENTS
      CALL RFILE(ITAP14)
C         GET GEOM AND GRADIENT POINTERS
      CALL WREADW(ITAP14,ISTART,1,500+NCALC,JUNK)
      IPNTG=ISTART+80
      IPNTN=ISTART+60+NACOUT-1
      CALL WREADW(ITAP14,IPNTN,1,IPNTN,JUNK)
      WRITE(IOUT,1010)IPNTG,IPNTN
C         GET RAW GRADIENTS
      IF(IPNTN.EQ.0)THEN
      WRITE(IOUT,1024)ITAP14
      CALL EXIT(100)
      ENDIF
      CALL WREADW(ITAP14,LACME,INTOWP(3*NATOMS),IPNTN,JUNK)
...
C        REORDER SYMMETRIC BASIS FUNCTIONS TO PUT FIXED CENTER LAST
      ...
      DO 10 I=1,NATM3
C         READ CHARACTERS
      READ(I76,1006)I,(SCR(J),J=1,NOP+3)
      DO 311 J=1,NOP
