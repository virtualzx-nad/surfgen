...
CCLM: removed. unnecessary. will define in subroutine
C      DIMENSION RS8(100)                                                 
C      DIMENSION IRS8(2)
C      EQUIVALENCE(RS8(1),IRS8(1)) 
      DATA NCOR/1500000/ 
      REAL*4 XT0,XU0,XS0,XT1,XU1,XS1,XT,XU,XS
      CHARACTER*(24) FDATE
      DATA IFAIL/0/
C-------------------------------------------------------
CCLM: calls not necessary, so we are removing them                        
C      CALL GETSIZ(ICORE)
C      CALL GETMEM(ICORE,IRS8,IAP,IFAIL)
C      CALL GETMEM(ICORE,IRS8,IAP)
C      IF(IFAIL.NE.0)GO TO 900
C      NCOR=ICORE-1
C      IAP=(IAP+2)/2
C---------------------------------------------------------
      IAP=1
C                                                       
      NPFLG=2 
C---------------------------------------------------------
CCLM: systime call not necessary so we are removing them
C      CALL SYSTIME(XT0,XU0,XS0)
C---------------------------------------------------------
C
      WRITE(6,*)' POLYHES CLOCK STARTS: ',FDATE()
C call to main polyhes driving subroutine
      print *, " ALLOCATING 2*", NCOR
      CALL VMAIN( NCOR )
      WRITE(6,*)' POLYHES CLOCK ENDS:   ',FDATE() 
C---------------------------------------------------------
CCLM
C      CALL SYSTIME(XT1,XU1,XS1)
C---------------------------------------------------------
C
      XT=XT1-XT0
      XU=XU1-XU0
...
CCLM: this print is for debugging
      ...
      PRINT *, "PRINTING LOCATION OF HESSS", HESSS
      PRINT *, "PRINTING IA(HESSS)", IA(HESSS)
...
C         used: SCGD1 SCENG
      dimension JGRADS(30,3,3), SCGD1(30,3,3), SCENG(10)
...
C
  ...
  130 CONTINUE
C
C CLM: Call surfgen for energies,gradients,etc
      call initPotential()
      JGRADS=0d0
      call EvaluateSurfgen77(NATOMS,3,GM,EVAL,JGRADS,
     1     SCGD1,SCENG)
      PRINT *, "SURFGEN CALL SUCCESSFUL"
...
C      WRITE(IOUT,1003)'E-DIF ',(RMACME(I),I=1,3*NATOMS)
C
...
C      WRITE(IOUT,1003)'H(IJ) ',(RLACME(I),I=1,3*NATOMS)
C      ENDIF
C
   50 CONTINUE
